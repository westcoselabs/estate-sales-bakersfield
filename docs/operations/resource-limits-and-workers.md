...
Following the owner's Pro upgrade and live-launch request, `vercel.json` schedules maintenance every five minutes and email every five minutes offset by two minutes. These schedules take effect only after deploying this configuration on Pro. Set Production `JOB_MAX_QUEUE_DELAY_MINUTES=15` with that deployment; the parser retains its legacy 1,500-minute fallback for existing environments. Normal Stripe webhook fulfillment and direct authentication email delivery do not depend on these cron runs. Raising cadence does **not** provide a high-volume continuous worker or a delivery-time guarantee. Verify hosted invocations and queue drain after deployment.
