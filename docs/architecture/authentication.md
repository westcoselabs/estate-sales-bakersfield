...
Sessions contain seven-day absolute expiration and never slide. Raw tokens exist only in the request cookie and session grant; PostgreSQL stores SHA-256 hashes. Preview, staging, and Production cookies are host-only, HTTP-only, Secure, `SameSite=Lax`, path `/`, and use the `__Host-` prefix.
