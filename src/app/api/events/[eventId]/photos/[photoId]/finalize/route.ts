...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const input = photoFinalizationSchema.parse(await readJson(request));
    const event = await createConfiguredEventService().finalizePhoto(
      await requireUser(),
      eventId,
      photoId,
      input,
    ...
    return eventApiError(error, request, "events.finalize-photo");
  }
}
