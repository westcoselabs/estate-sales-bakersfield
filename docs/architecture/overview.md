# Current Architecture Through Phase 4
...
The baseline includes validated environment configuration, CSP and standard security headers, trusted-origin checks, Pino/Sentry redaction, narrow principals, Argon2id, hashed opaque tokens, transactional expiry/revocation/rotation, enumeration-safe responses, provider-neutral email/rate-limit/media/location/payment ports, private address projections, owner/admin draft-media authorization, immutable processed media keys, optimistic event versions, append-only audit history, deterministic revision approval digests, raw-body webhook signature verification, server-authoritative pricing, and idempotent webhook/reconciliation fulfillment.

## Scope boundary

The application stops after the first complete paid-publication proof: one-time hosted Checkout, webhook-authoritative fulfillment, stable public detail pages, and durable reconciliation. Search/discovery, maps, imports, favorites, refunds UI, coupons, packages, subscriptions, Stripe Connect, marketplace payouts, paid editing, and Production launch remain outside Phase 4.
