...
export async function POST(
  ...
  try {
    ...
    const result =
      await createConfiguredListingImportReviewService().approveCandidate(
        actor,
        candidateId,
    ...
    return adminJson(
      { schema: "listing-import-candidate-approval.v1", ...result, requestId },
      { requestId },
    );
  } catch (error) {
    return adminApiError(
      error,
      requestId,
      "admin.imports.candidates.approve",
      startedAt,
    );
  }
}
